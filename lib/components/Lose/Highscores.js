...
function addSubmitScore() {
    ...
    if (totalTimer > scores[0]) {
      ...
      while (i--) {
        ...
        if (totalTimer >= thisScore) {
          insertIndex = i;
          console.log(totalTimer + ' is greater than ' + thisScore);
  ...
  if (insertIndex !== null) {
    ...
    checkMaxScores();
    if ($('.score').eq(insertIndex).length !== 0 && scores.length >= 10) {
      $('.score').eq(insertIndex).replaceWith(`
      <div class='score' id="new-score" style="${LosePromptStyles['score']}">
        <input type='text' placeholder='Enter your name.' maxlength="16" class='name textinput' style="${LosePromptStyles[
          'scoreText'
        ]}color:black;" />
