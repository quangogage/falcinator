...
// ** Global Functions ** \\
export function shootBullet(mouseX, mouseY, startX, startY, world, isBullet) {
  var shootFunc = function(angle, startX, startY, shakeIntensity) {
    var shakeIntensity = shakeIntensity || 3.2;
    var bulletEl = $(`<div class='bullet'></div>`);
    var isCannonball;
    if (!isBullet && powerupList[5].active) {
      bulletEl = $(`<img src=${cannonball} class="cannonball"/>`);
      isCannonball = true;
    }
  ...
  HandleBurstShot(angle, startX, startY, shootFunc);
}
