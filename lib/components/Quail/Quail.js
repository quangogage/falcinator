...
import { AddTime } from '../Timer/Timer';
...
var timeMinus = -200; // How much time is decreased when a quail travels across the entire page?
  ...
  for (var i = quails.length - 1; i >= 0; i--) {
    ...
    for (var ia = bullets.length - 1; ia >= 0; ia--) {
      ...
      if (
        ...
        addScore(1);
        CreateFlash(
          va.x + Math.cos(va.angle) * va.el.height() / 2,
          va.y + va.el.height() / 2 + Math.sin(va.angle) * va.el.height() / 2
        );
        AddTime(timeAdd);
        if (va.onDestroy) {
